...
def collect_and_store(agent: Agent, buffer: ReplayCache, critic: Critic,
                      traj_id: int, tms_clk: int, input_model,
    ...
    buffer.add_sample(
        state=input_model.squeeze(),
        action=agent.a_t,
        reward=reward,
        log_prob=log_prob.cpu().detach().numpy(),
        value=critic(input_model).cpu().detach().numpy().reshape(-1)[0],
