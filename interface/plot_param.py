...
def plot_param_process(path_dir, filename='training_metrics_all.pth',update_interval=2000):
    ...
    lines = {
        ...
        "entropy": axs[0, 1].plot([], [], label="Iterations", color='orange')[0],
    ...
    def update(frame):
        try:
            ...
            lines["entropy"].set_data(new_iterations, new_entropy)
            lines["actor_loss"].set_data(new_iterations, new_actor_loss)
            lines["critic_loss"].set_data(new_iterations, new_critic_loss)
